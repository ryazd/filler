...
/*   Updated: 2020/10/26 22:02:44 by hluthor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				check_valid_length(t_filler **filler, int i, int j, int width)
{
	...
	{
		j1 = 0;
		while (j1 < (int)ft_strlen((*filler)->piece[0]))
		{
			if (!checker((*filler)->piece[i1][j1],
						(*filler)->plateau[i][j], filler, 1))
				return (0);
			if (checker((*filler)->piece[i1][j1],
						(*filler)->plateau[i][j], filler, 2))
				ok++;
			inc(&j1, &j);
		}
		j = j_copy;
